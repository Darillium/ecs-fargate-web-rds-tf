...
data "template_file" "web_task" {
  ...
  vars {
    #image           = "${aws_ecr_repository.petclinic_app.repository_url}"
    image           = "barsutka/darillium-petclinic"
    # secret_key_base = "${var.secret_key_base}"
    database_url    = "jdbc:mysql://${var.database_endpoint}:3306/${var.database_name}?encoding=utf8&pool=40"
...
data "template_file" "db_init_task" {
  template = "${file("${path.module}/tasks/db_init_task_definition.json")}"

  vars {
    image           = "barsutka/petclinic-db-init:latest"
...
resource "aws_ecs_task_definition" "db_init" {
  family                   = "${var.environment}_db_init"
  container_definitions    = "${data.template_file.db_init_task.rendered}"
  requires_compatibilities = ["FARGATE"]
  network_mode             = "awsvpc"
  cpu                      = "256"
  memory                   = "512"
...
resource "aws_alb" "alb_petclinic" {
  name            = "${var.environment}-alb-petclinic"
...
resource "aws_alb_listener" "petclinic" {
  load_balancer_arn = "${aws_alb.alb_petclinic.arn}"
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_iam_role_policy" "ecs_service_role_policy" {
  ...
  #policy = "${file("${path.module}//ecs-service-role.json")}"
...
#   alarm_name          = "${var.environment}_petclinic_web_cpu_utilization_high"
